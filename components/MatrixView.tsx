      ...
      <div className="overflow-auto matrix-scroll flex-1 min-h-0">
        <table className="min-w-full border-collapse text-sm" style={{ zoom: zoomLevel / 100 }}>
          <thead className="bg-slate-50">
            <tr>
              <th className="sticky top-0 left-0 z-50 bg-slate-50 p-3 text-left font-semibold text-slate-600 border-b border-r border-slate-200 w-24 min-w-[100px] flex items-center gap-2">
                 Unit
              </th>
              {interiorTrades.map((trade, index) => (
                <React.Fragment key={trade.id}>
                  ...
                  {index === 0 && onAddTrade && !isReordering && (
                    <th 
                      className="sticky top-0 z-40 p-0 border-b border-slate-200 w-1 min-w-[8px] relative group bg-slate-50"
                  ...
                  <th 
                    ...
                    className={`sticky top-0 z-40 p-2 text-center font-medium text-slate-600 border-b border-slate-200 min-w-[120px] whitespace-nowrap bg-slate-50 ${isReordering ? 'cursor-move hover:bg-indigo-50' : (onDeleteTrade || onEditTrade) ? 'cursor-pointer hover:bg-slate-100' : ''}`}
                  >
          ...
          <tbody>
            {interiorUnits.map((unit, index) => (
              ...
              >
                <td 
                  className={`sticky left-0 z-30 bg-white p-3 font-medium text-slate-700 border-b border-r border-slate-200 shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)] ${!isReordering && onEditUnit ? 'cursor-pointer hover:bg-slate-50' : ''}`}
